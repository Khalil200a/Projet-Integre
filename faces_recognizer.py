...
def create_face_encodings(frame):
    ...
    try:
        if frame is not None:
            locate_faces = face_recognition.face_locations(frame, model=MODEL,
                                                           number_of_times_to_upsample=2)  # find all faces' locations in the frame
            face_encoded = face_recognition.face_encodings(frame,
                                                           num_jitters=2)  # create the encodings for all found faces in the frame
