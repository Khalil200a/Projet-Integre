...
# file that contains all the read/write functionalities
import file_handlers
import os

cap = cv2.VideoCapture(0)

##APP_ICON_PATH = "~/Desktop/security_cam/images/iconEE.ico" #GUI icon path
PASSWORD_ICON_PATH = os.path.dirname(os.path.realpath(__file__)) + "/images/password_iconEE.png"  # password icon path
...
canvas = tk.Canvas(root, width=WIDTH - 5, height=HEIGHT - 5, bg="blue")
canvas.place(relx=0.03, rely=0.052)

recognition_button = tk.Button(canvas, text="reconn.Faci. ", command=enable_recognition,
...
MESSAGE.set("Pour Ajouter un Utilisateur,\nenter le mot de passe!")
message_label = tk.Label(root, textvariable=MESSAGE, wraplength="5c", bg="#0DEEEC", fg="#BE2191")
...
second_seperator.place(relx=0.98, rely=0.265, relwidth=0.22, anchor="ne")

separator = ttk.Separator(root, orient='vertical')
separator.place(relx=0.76, rely=0.265, relwidth=0.22, relheight=1)
...
show_password_button = tk.Button(root, command=show_password, image=password_icon, border=0, bg="#131113",
                                 activebackground="#131113")
...
third_seperator.place(relx=0.98, rely=0.570, relwidth=0.22, anchor="ne")
